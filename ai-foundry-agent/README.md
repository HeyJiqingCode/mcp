...
## Foundry Agent SDK Sample

If you want to create a Microsoft Foundry agent in code and attach a remote MCP server, use the TypeScript sample in `samples/foundry-agent-sdk-ts`.

The sample is preconfigured for this workspace's validated remote MCP endpoint:

- `http://20.205.26.133/azure-search-mcp`

It uses `@azure/ai-projects` with `DefaultAzureCredential` and the Foundry project settings provided during validation.
